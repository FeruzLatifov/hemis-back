...
- **Webhook Outbound Infrastructure** (ADR-0012): Markaz → 224 Univer real-time event push. 5-bosqichli implementatsiya (Sprint 1-5):
  - **Sprint 1 (Foundation):** V016 migration (`webhook_target` + `webhook_delivery_log` jadvallar + 7 ta index), `OutboxEvent` JPA entity (V015 outbox_event'ga map), `WebhookTarget`/`WebhookDeliveryLog` entitylar + repository, `spring-kafka` dependency + `application.yml` Kafka config (idempotent producer, manual ack consumer).
