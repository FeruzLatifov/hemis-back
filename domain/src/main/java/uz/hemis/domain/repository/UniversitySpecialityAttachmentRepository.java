...
public interface UniversitySpecialityAttachmentRepository extends JpaRepository<UniversitySpecialityAttachment, UUID> {
    ...
    /**
     * How many attachments this speciality has — the classifier delete guard counts these.
     *
     * <p>Every physical row counts: the table has no soft delete, so what the admin sees in the
     * registry is exactly what {@code fk_univ_spec_attach_spec} ({@code ON DELETE RESTRICT}) blocks
     * on. A derived query is enough — there is no predicate left to spell out.</p>
     */
    long countBySpecialityId(UUID specialityId);

    /**
     * The same truth as {@link #countBySpecialityId}, but GROUPED BY OTM — the list of universities
     * that block a classifier delete, one row each, ordered by OTM code.
     *
     * <p>Grouped rather than row-per-attachment because the admin needs to know <em>where to go</em>
     * (the OTM), not how many form/year rows are there. Same predicate as the counter, so the count
     * in the 422 message and the list in the dialog can never disagree.</p>
     *
    ...
    @Query(value = "SELECT university_code, COUNT(*) " +
           ...
           nativeQuery = true)
    List<Object[]> countBySpecialityIdGroupedByUniversity(@Param("specialityId") UUID specialityId);
    ...
    /** Distinct OTM codes that have at least one attachment. */
    ...
    /**
     * Duplicate guard: an attachment for the same (OTM, speciality, education_form, edu_year),
