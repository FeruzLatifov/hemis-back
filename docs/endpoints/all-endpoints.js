        const endpoints = [
            ...
            // ============================================
            // 03.Passport ma'lumotlari (1 endpoint)
            // ============================================
            {
                id: 1,
            ...
            // ============================================
            // 03.Passport ma'lumotlari (endpoint #7)
            // ============================================
            {
                id: 2,
                category: "03.Passport ma'lumotlari",
            ...
            // ============================================
            // 04.Talaba (Entity API - CRUD ketma-ketligi)
            // ============================================
            // TEST OQIMI: 1️⃣ POST (yaratish) → 2️⃣ GET (olish) → 3️⃣ PUT (yangilash) →
            //             4️⃣-17️⃣ Boshqa operatsiyalar → 18️⃣ DELETE (o'chirish - OXIRIDA!)
            // ⚠️ MUHIM: DELETE ni faqat barcha testlar tugagandan keyin ishga tushiring!
            // ============================================
            {
                id: 1,
                category: "04.Talaba",
            ...
            // ============================================
            // Qo'shimcha Talaba endpointlari (4-18)
            // ============================================
            {
                id: 4,
                category: "04.Talaba",
            ...
            // =====================================================
            // 04.Talaba - StudentMeta Entity (hemishe_EStudentMeta)
            // =====================================================
            {
                id: 13,
                category: "04.Talaba",
            ...
            // ============================================
            // 🗑️ OXIRGI QADAM - Test talabani o'chirish
            // ============================================
            {
                id: 18,
                category: "04.Talaba",
            ...
            // ═══════════════════════════════════════════════════════════════════════════════════════
            // 05.O'qituvchi (Entity API - hemishe_ETeacher)
            // ═══════════════════════════════════════════════════════════════════════════════════════
            {
                id: 1,
            ...
            // ============================================
            // 06.Xodim lavozimlari (6 endpoints)
            // Ikkala sistema bitta bazaga ulangan - shuning uchun:
            // 1. Avval GET all orqali bazadan birinchi ID olinadi
            // 2. Keyin o'sha ID bilan GET/PUT/DELETE qilinadi
            // ============================================
            {
                id: 1,
            ...
            },
            {
                id: 4,
                category: "06.Xodim lavozimlari",
                name: "Yangi xodim lavozimi yaratish (Entity API)",
                method: "POST",
                url: "/app/rest/v2/entities/hemishe_EEmployeeJobs",
                requiresAuth: true,
                inputFields: {
                ...
                // MUHIM: _department FK sifatida hemishe_e_university_department.code ga bog'langan!
                // Shuning uchun bazada mavjud department kodi ishlatilishi kerak
                bodyGenerator: () => ({
                    "_university": "401",
                    "_department": "401-101",  // Bazada mavjud: 401-101, 401-101-01, 401-101-02, 401-101-03
                    "_employeeType": "12",     // Bazadan: 12
                    "_employeePosition": "13", // Bazadan: 13, 14
                    "_employeeRate": "13",     // Bazadan: 11, 13
                    "_employeeForm": "12",     // Bazadan: 11, 12
                    "_employeeStatus": "11",   // Bazadan: 11
                    "jobStartDate": new Date().toISOString().split('T')[0],
                    "contractDate": new Date().toISOString().split('T')[0],
                    "contractNumber": "NEW-" + String(Math.floor(Math.random() * 9000) + 1000) + "/2025",
                    "decreeDate": new Date().toISOString().split('T')[0],
                    "decreeNumber": "BQ-" + String(Math.floor(Math.random() * 900) + 100),
                    "tag": "test-" + Date.now()
                }),
                description: "Yangi xodim lavozimi yaratish. Dinamik qiymatlar generatsiya qilinadi.",
                dependsOn: 1,
                ported: true
            },
            {
            ...
            // ============================================
            // 07.OTM bo'linmalari (Entity API)
            // ============================================
            {
                id: 1,
...
<b>⚠️ Eslatma:</b> Avval #2 orqali test OTM yarating, keyin shu endpoint orqali o'chiring!`,
            ...
            // =====================================================
            // 10.Talaba holati - Talaba holatlari klassifikatori
            // =====================================================
            {
                id: 1,
            ...
            // =====================================================
            // 11.Fuqarolik holatlari - Fuqarolik klassifikatori
            // =====================================================
            {
                id: 1,
            ...
            // ============================================
            // 12.Diplomlar - Talaba diplomlari (EStudentDiploma)
            // Tartib: 1.POST → 2.PUT → 3.DELETE → 4.GET by ID → 5.GET all → 6-7.Search
            // ============================================
            {
                id: 1,
            ...
            },
            {
                id: 7,
                category: "12.Diplomlar",
                name: "Diplomlarni qidirish (POST /search)",
                method: "POST",
                url: "/app/rest/v2/entities/hemishe_EStudentDiploma/search",
                requiresAuth: true,
                separateInputs: true,
                ...
                hasBody: true,
                // CUBA POST /search: body = {"filter": {...}}, limit/offset = query params
                bodyGenerator: (inputs) => {
                    try {
                        // Filter stringni JSON ga parse qilib, "filter" wrapper ichiga o'rash
                ...
                // limit va offset query param sifatida
                queryParamsFromInputs: ["limit", "offset", "returnNulls"],
                description: "CUBA filter formatida qidirish. Body = {filter: {...}}, limit/offset = query param.",
                ported: true
            },
            // ============================================
...
<b>Response:</b> Massiv formatida, to'liq nested objectlar bilan`,
            ...
            // ============================================================
            // 18.Ilmiy faollik - Research Activity Entity API
            // ============================================================
            {
                id: 1,
...
<b>⚠️ Eslatma:</b> Soft delete - delete_ts ustuni belgilanadi!`,
            ...
            // ============================================================
            // 22.Ilmiy nashrlar - Publication Scientific Entity API
            // ============================================================
            {
                id: 1,
...
<b>⚠️ Eslatma:</b> Soft delete - delete_ts ustuni belgilanadi!`,
            ...
            // ============================================
            // 23.Ilmiy ishlanmalar (Patent, Ixtiro, Foydali model)
...
<b>Response:</b> Filter shartiga mos yozuvlar`,
            ...
            // Entity: hemishe_RAcademicGroup
            // Table: hemishe_r_academic_group
            {
                id: 1,
...
<b>Response:</b> Filter shartiga mos yozuvlar`,
            ...
            // Entity: hemishe_RAcademicSubjects
            // Table: hemishe_r_academic_subjects
            {
                id: 1,
...
<b>Response:</b> Filter shartiga mos yozuvlar`,
            ...
            // Entity: hemishe_RAcademicScore
            // Table: hemishe_r_academic_score
            // Ketma-ketlik: POST → GET by ID → PUT → DELETE → GET all → Search (GET/POST)
            // =====================================================
            {
                id: 1,
...
<b>Response:</b> Filter shartiga mos yozuvlar`,
            ...
            // Entity: hemishe_RAcademicAttendance
            // =====================================================
            {
                id: 1,
...
<b>Response:</b> Saqlangan statistika ma'lumotlari`,
            ...
            // Entity API: hemishe_RContractStatistics
            {
                id: 2,
                category: "36.Shartnoma statistikasi",
...
<b>Response:</b> Shartnoma statistikasi ro'yxati (nested objectlar bilan)`,
            ...
            // Entity: hemishe_REmployment
            // Table: hemishe_r_employment
            // Ketma-ketlik: POST → GET by ID → PUT → GET list → DELETE → SEARCH
            // =====================================================
            {
                id: 1,
