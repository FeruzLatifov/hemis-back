...
public class JwtTokenProvider {
     ...
     * @param username     Username
     * @param universityId University ID (for multi-tenant)
     * @param permissions  User permissions (entity:action format)
     * @return JWT access token
     */
    public String generateToken(String userId, String username, String universityId, java.util.Set<String> permissions) {
        Date now = new Date();
        Date expiryDate = new Date(now.getTime() + tokenExpiration);
        ...
        String token = Jwts.builder()
                ...
                .claim("username", username)
                .claim("universityId", universityId)
                .claim("permissions", new java.util.ArrayList<>(permissions))
                .claim("type", "access")
                .issuedAt(now)
                .expiration(expiryDate)
                .signWith(secretKey, Jwts.SIG.HS512)
        ...
        log.debug("Generated access token for user: {} with {} permissions (expires: {})",
                username, permissions.size(), expiryDate);
        return token;
    }
     ...
     *
     * @param token JWT token
     * @return Set of permission strings (entity:action format)
     */
    @SuppressWarnings("unchecked")
    public java.util.Set<String> getPermissionsFromToken(String token) {
        Claims claims = parseClaims(token);
        java.util.List<String> permissionsList = claims.get("permissions", java.util.List.class);
        return permissionsList != null ? new java.util.HashSet<>(permissionsList) : java.util.Collections.emptySet();
    }
