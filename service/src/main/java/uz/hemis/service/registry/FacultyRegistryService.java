...
public class FacultyRegistryService {
    ...
    public Page<FacultyGroupRowDto> getFacultyGroups(String q, Boolean status, Pageable pageable) {
        ...
        // filtrga mos fakulteti yo'q OTM ro'yxatda ko'rinmasin (aks holda foydalanuvchi
        // qatorni ochadi-yu ichi bo'sh chiqadi). Ichki ro'yxat getFacultiesByUniversity'da allaqachon filtrlanadi.
        if (status != null) {
            where.append("AND EXISTS (SELECT 1 FROM hemishe_e_university_department d "
                    + "WHERE d.university_code = u.code AND d._deparment_type = '11' "
